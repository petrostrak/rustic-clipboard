...
use crate::web::{ctx, form, hitcounter::HitCounter, renderer::Renderer, PageError};
...
fn home(renderer: &State<Renderer<'_>>) -> RawHtml<String> {
    let contecx = ctx::Home::default();
    RawHtml(renderer.render(contecx, &[]))
}
...
pub async fn new_clip(
    ...
    if let Some(value) = form.value {
        ...
        Err((
            Status::BadRequest,
            RawHtml(renderer.render(ctx::Home::default(), &errors)),
        ))
    }
}
...
pub async fn submit_clip_password(
    ...
    if let Some(form) = &form.value {
        ...
        Ok(RawHtml(renderer.render(
            context,
            &["A password required to view this clip"],
        )))
    }
}
...
pub async fn get_raw_clip(
    ...
    let req = service::ask::GetClip {
        ...
        password: cookies
            ...
            .map(|raw_pass| Password::new(raw_pass.to_string()).ok())
            .flatten()
